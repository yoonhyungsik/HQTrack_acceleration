...
class AOTTracker(object):
    ...
    def track(self, image):
        ...
        for aug_idx in range(self.aug_nums):
            ...
            save_dir = r'C:\Users\ye761\HQTrack\demo\p_30_pred_logit.path'  # Update this to your desired directory
            os.makedirs(save_dir, exist_ok=True)
            counter=1
            for _ in range(len(img_files)):
                save_path = os.path.join(save_dir, f"pred_logit_frame{counter}_aug{aug_idx}.pt")  # Include frame_idx and aug_idx
                torch.save(pred_logit.cpu(), save_path)
                # Alternatively, you can save as a numpy file
                # np.save(save_path.replace('.pt', '.npy'), pred_logit.cpu().numpy())
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                pred_logit = pred_logit.to(device)
                counter += 1 
        ...
        conf = 0
        print("Returning from track:", type(mask), type(conf))
        return mask, conf
