...
cur_colors = [(0, 255, 255), (255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 255), (0, 0, 0), (255, 255, 0), (225, 228, 255), (180, 105, 255), (255, 0, 255)]*100
    ...
    def track(self, image):
        ...
        if self.sam_refine:
            ...
            for i in obj_list:
                ...
                #iou = iou.cpu().numpy()
                # IoU 계산
                output_tensor = torch.from_numpy(output).cuda().unsqueeze(0)
                mask_tensor = torch.from_numpy(mask).cuda().unsqueeze(0)
                obj_nums = torch.tensor([1], device='cuda')
                
                iou = pytorch_iou(output_tensor, mask_tensor, obj_nums)
                print(f"Object {i} IoU score: {iou.item():.4f}")
...
for idx, img_file in enumerate(img_files):
    ...
    if idx == 0:
        ...
        depth_map = torch.from_numpy(depth_map).unsqueeze(0).unsqueeze(0).float().cuda()
        ...
        # 깊이 맵을 pred_logit에 합치기
        pred_logit = tracker.tracker.engine[0].decode_current_logits((img_ori.shape[0], img_ori.shape[1]))
        pred_logit = pred_logit + depth_map  # 깊이 맵 정보를 pred_logit에 추가
        '''
        #pred_logit시각화
        import matplotlib.pyplot as plt

        # pred_logit을 확률 값으로 변환
        pred_prob = torch.softmax(pred_logit, dim=1)

        # 각 픽셀에 대해 가장 높은 확률을 가진 클래스 선택
        pred_label = torch.argmax(pred_prob, dim=1).squeeze().cpu().numpy()

        # 시각화
        plt.figure(figsize=(10, 10))
        plt.imshow(pred_label, cmap='jet')  # 'jet' 컬러맵을 사용하여 클래스를 시각화
