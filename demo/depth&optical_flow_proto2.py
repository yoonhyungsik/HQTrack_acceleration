...
class MotionDepthAnalyzer:
    def __init__(self):
        self.depth_history = {}  # {obj_id: deque([...])}
    ...
    def update_depth_history(self, obj_id, depth_value):
        if obj_id not in self.depth_history:
            self.depth_history[obj_id] = deque(maxlen=self.window_size)
        self.depth_history[obj_id].append(depth_value)
    ...
    def should_skip_tracking(self, obj_id, depth_value, motion_mag):
        if obj_id not in self.depth_history or len(self.depth_history[obj_id]) < 3:
            return False

        depth_hist = list(self.depth_history[obj_id])
        last_depth = depth_hist[-1]
        depth_diff = abs(depth_value - last_depth)

        depth_thresh = np.std(depth_hist) * 1.5 + 0.01
        motion_thresh = 0.5

        skip = (depth_diff < depth_thresh) and (motion_mag < motion_thresh)

        print(f"[Analyzer] Obj {obj_id} | Depth Δ: {depth_diff:.4f} (th: {depth_thresh:.4f}), Motion: {motion_mag:.4f} (th: {motion_thresh:.2f}) → Skip: {skip}")
        return skip

analyzer = MotionDepthAnalyzer()
'''
...
class HQTrack(object):
    ...
    def track(self, image):
        ...
        for obj_id in range(self.obj_num):
            ...
            # 뎁스 변화량이 작으면 트래킹 스킵
            if analyzer.should_skip_tracking(obj_id, obj_depth):
                print(f"Skipping tracking for object {obj_id}")
                continue

            m, confidence = self.tracker.track(image)
            analyzer.update_depth_history(obj_id, obj_depth)
            all_masks.append(m)
...
for idx, img_file in enumerate(img_files):
    ...
    if idx == 0:
        ...
        for obj_id in range(obj_num):
            analyzer.update_depth_history(obj_id, np.mean(depth_map[rs == (obj_id+1)]))
        last_mask = rs.copy()
        ...
        should_track_any = False
        for obj_id in range(obj_num):
            obj_mask = (last_mask == (obj_id + 1)) if 'last_mask' in locals() else np.zeros_like(depth_map, dtype=bool)
            obj_depth = np.mean(depth_map[obj_mask]) if np.any(obj_mask) else 0.0
            motion_vector = analyzer.compute_optical_flow(prev_frame, img_ori, obj_mask) if idx > 0 else (0.0, 0.0)
            motion_mag = analyzer.compute_motion_magnitude(motion_vector)
            if not analyzer.should_skip_tracking(obj_id, obj_depth, motion_mag):
                should_track_any = True
                break

        if not should_track_any:
            print(f"[SKIP] Skipping entire frame {idx} due to low variation")
            continue

        m, confidence = tracker.track(img_ori)
        print(f"[TRACK] Frame {idx}")
        
        # Fuse depth and motion into predicted logits
        ...
        for obj_id in range(obj_num):
            ...
            motion_mag = analyzer.compute_optical_flow(prev_frame, img_ori, obj_mask) if idx > 0 else 0.0

            if obj_id in analyzer.depth_history and len(analyzer.depth_history[obj_id]) > 0:
                depth_diff = float(abs(np.mean(depth_region) - analyzer.depth_history[obj_id][-1]))
            else:
                depth_diff = 1e-6
                
            if obj_id in analyzer.motion_history and len(analyzer.motion_history[obj_id]) > 0:
                motion_diff = float(abs(motion_mag - analyzer.motion_history[obj_id][-1]))
            else:
                motion_diff = 1e-6
                
            depth_diff = float(depth_diff)
            motion_diff = float(motion_diff)
            total = depth_diff + motion_diff + 1e-6
            w_depth = depth_diff / total
            w_motion = motion_diff / total

            depth_tensor = torch.from_numpy(depth_map[obj_mask].astype(np.float32)).cuda()
            ...
            fusion_val_gpu = fusion_val.cuda() if temp_tensor.is_cuda else fusion_val
            temp_tensor[obj_mask] = fusion_val_gpu
        ...
        for obj_id in range(obj_num):
            ...
            if prev_frame is None or prev_frame.size == 0:
                print(f"[WARN] prev_frame is invalid at frame {idx}, skipping optical flow.")
                motion_vector = (0.0, 0.0)
            else:
                try:
                    motion_vector = analyzer.compute_optical_flow(prev_frame, img_ori, obj_mask)
                except cv2.error as e:
                    print(f"[CV2 ERROR] Optical flow failed at frame {idx} for object {obj_id}: {e}")
                    motion_vector = (0.0, 0.0)
